...
### Capture — Persistance réelle dans MongoDB Atlas

Le monitoring est stocké de façon durable dans MongoDB (base `medical_supervisor_agent`, collection `runs`), et non plus uniquement en mémoire — l'historique survit donc aux redémarrages et redéploiements du service sur Railway.

![Collection runs dans MongoDB Atlas](docs/images/mongodb_collection_runs.png)

*Capture du Data Explorer MongoDB Atlas montrant la collection `runs` (4 documents) à l'intérieur de la base `medical_supervisor_agent`, avec un document détaillé : `correlation_id`, `status: "completed"`, `events` (tableau de 10 événements, un par nœud du graphe), et `summary` (`risk_label: "Bas"`, `final_alert`).*
